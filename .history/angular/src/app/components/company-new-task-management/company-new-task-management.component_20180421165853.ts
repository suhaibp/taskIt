...
export class CompanyNewTaskManagementComponent implements OnInit {
  ...
  approve(){
    ...
    let module =[] 
    this.task = {modules:[this.taskRequest.tbl_project_module], teamMembers:[this.taskRequest.tbl_user_profile]}
    // let tbl_estimation_tasks = {tbl_estimation_tasks:[]}
    this.task.modules[0].tbl_estimation_tasks=[this.taskRequest];
    this.task.teamMembers[0].start_date=[this.taskRequest];
  ...
  getLastTaskDetails(){
    ...
    this.companyService.getLastTaskDetails(this.projectId, assignId).subscribe(res => {
      this.lastTask = res;
      let lastDate = new Date(res[0].tbl_project_tasks[0].planned_end_date_time)
