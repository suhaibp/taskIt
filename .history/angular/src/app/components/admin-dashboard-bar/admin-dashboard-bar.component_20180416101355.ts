...
export class AdminDashboardBarComponent implements OnInit {
    ...
    this.superAdminService.getBarDataforAdminDashboard().subscribe(data=>{
      ...
      if(data.length>0){
        data.forEach(element => {
          ...
          this.offensesByYear.push({"count":5})
        });
    this.bargraph()
  ...
  bargraph(){
    ...
    let n     = this.offenseNames.length, // number of layers
        m     = this.offensesByYear.length, // number of samples per layer
        stack = d3.stack().keys(this.offenseNames);
    
    let layers = stack(this.offensesByYear); // calculate the stack layout
    
    layers.forEach(function(d, i) {
      // add keys to every datapoint
      d.forEach(function(dd, j) {
        dd.year = this.years[j];
        dd.offenseName = this.offenseNames[i];
