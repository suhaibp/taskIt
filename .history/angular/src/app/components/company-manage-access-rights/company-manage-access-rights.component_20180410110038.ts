...
export class CompanyManageAccessRightsComponent implements OnInit {
  ...
  getBooks(): TreeviewItem[] {
    ...
    this.accessRights.forEach(element => {
      ...
      if(element.checked == true){
        temp = {text:element.name,collapsed:true,value:element.id, children:[], checked:true};

      }
      else{
        temp = {text:element.name,collapsed:true,value:element.id, children:[], checked:false};
        
      }
       element.sub.forEach(ele => {
         if(ele.checked == true){
          temp.children.push({text: ele.access_right, value:ele.id ,checked:true})
         }
         else{
          temp.children.push({text: ele.access_right, value:ele.id ,checked:true})
         }
       });
       temp = new TreeviewItem(temp);
       array.push(temp);
    });
    console.log(array);
  ...
  //  ---------------------------------end-----------------------------------------------
  change(downlineItems){
    console.log(downlineItems[1].parent.item.value)
    let allChecked = true;
